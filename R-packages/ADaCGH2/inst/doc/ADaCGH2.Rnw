...
can also be used (our servers use LAM-MPI). In this example we will use 4 nodes.

<<>>=
snowfallInit(universeSize = 4, typecluster = "SOCK")
@ 
...
\texttt{pSegment} has been called using \option{merging = "none"}.
...
<<>>=
inputDataToADaCGHData(MAList = MA, na.omit = TRUE)

inputDataToADaCGHData(MAList = MA2, na.omit = TRUE, 
                      minNumPerChrom = 4)
@ 
...
Rout one: (viñeta snapCGH, p. 2 y 3)
<<>>=
...
From limma vignette (section 3.2, p.8). I change the name of objets
appendin ``.limma''. Same as above, not using 
<<>>=
library(limma)
targets.limma <- readTargets("targets.txt", path = datadir)
RG.limma <- read.maimages(targets.limma, path = datadir, source="genepix")
...
Using CGHregions. 
First, check we use calls or segments

<<>>=

data(WiltingCalled)
result <- CGHregions(WiltingCalled)
sm <- segmented(WiltingCalled)
ca <- calls(WiltingCalled)

## This works

cl2 <- data.frame(name = featureNames(WiltingCalled),
                  chromosome = chromosomes(WiltingCalled),
                  start = bpstart(WiltingCalled),
                  end = bpstart(WiltingCalled),
                  ca)
r3 <- CGHregions(cl2)
r3 ## compare to
result
...
## Using smoothed data does, of course, give different results:
sm2 <- data.frame(name = featureNames(WiltingCalled),
                  chromosome = chromosomes(WiltingCalled),
                  start = bpstart(WiltingCalled),
                  end = bpstart(WiltingCalled),
                  sm)
...
## And we do need the first four columns, as this thinks 
## there are only 4 samples

cl3 <- data.frame(name = featureNames(WiltingCalled),
                  chromosome = chromosomes(WiltingCalled),
                  start = bpstart(WiltingCalled),
                  ca)
r5 <- CGHregions(cl3)
...
idat <- data.frame(name = featureNames(WiltingCalled),
                   chromosome = chromosomes(WiltingCalled),
                   start = bpstart(WiltingCalled),
                   end = bpend(WiltingCalled))

sm1 <- data.frame(idat, sm)
cl1 <- data.frame(idat, ca)
