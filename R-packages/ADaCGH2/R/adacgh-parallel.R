...
## for each array, plot the observed and segmented data
plot.olshen <- function(res, arraynum, main = NULL,
...
## lots of ugly kulges here; for instance, plotting is not done just inside
## sw.plot2, but also outside of it!!! Uuughh...

my.sw <- function(logratio, chrom, sign = +1, p.crit = 0.1, main = NULL,
                  nIter = 10000, prec = 100, name, ...) {
    ## all run as one large single chromosome
    ...
    swt.run <- sw(swt, trace = FALSE)


    swt.perm <- sw.perm.test(swt, max.nIslands = NULL,
                             nIter = nIter)
    swt.rob <- sw.rob(swt, prec = prec)
    ...
    for(jj in 1:length(swt.perm)) {
        x0 <- swt.run$start[jj]
        x1 <- x0 + swt.run$length[jj] - 1
        if(x1 > length(logratio)) stop("x1 > length(logratio); should never happen")
        perm.p.values[x0:x1] <- swt.perm[jj]
    }

    ## The plotting proper
    

    webPNG(file = paste(name, ".png", sep = ""),
           width = png.width,
