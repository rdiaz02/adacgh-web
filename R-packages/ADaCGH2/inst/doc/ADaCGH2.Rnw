...
\section{Overview:}
ADaCGH2 is a package for the analysis of CGH data. ADaCGH2 provides the
segmentation routines for the waviCGH \cite{Carro2010} (\url{http://wavi.bioinfo.cnio.es})
web-server application for the analysis and visualization of array-CGH
data. 

The main features of ADaCGH2 are:
\begin{itemize}
...
\item Parallelization and \textit{ff} are used simultaneously. WaviCGH
  consitutes a clear demonstration of the usage of \textit{ff} on a
